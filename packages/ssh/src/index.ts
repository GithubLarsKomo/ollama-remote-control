...
import type { HostKeyObservation } from '@orc/core';
    ...
    try {
      client.connect({
        ...
        hostVerifier(key: Buffer) {
          ...
          // A probe observes the key but deliberately does not trust it.
          return false;
        },
      });
    } catch (error) {
...
export function verifyPrivateKeyAccess(
  ...
  return new Promise((resolve, reject) => {
    ...
    client
      .on('ready', () => {
        ...
        if (!result) {
          reject(new SshTransportError(
            'SSH_CONNECT_FAILED',
            'SSH connection became ready without a host-key observation.',
          ));
          return;
        }
        resolve(result);
      ...
      .on('error', (error: Error) => {
        reject(mapConnectionError(error, mismatch));
      });
