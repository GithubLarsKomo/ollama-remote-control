...
export interface ContainerUpdatePanelProps {
...
export default function ContainerUpdatePanel({ target, onBusyChange, onSignedOut, onUpdated }: ContainerUpdatePanelProps) {
    ...
    <section className="update-panel" aria-labelledby="container-update-title">
      <div className="update-heading">
        <div>
          ...
          <p className="muted">The server captures rollback state, resolves the registry candidate and validates Docker Compose before any replacement is allowed.</p>
        </div>
      ...
      {phase === 'idle' ? (
        <div className="update-action-row">
          ...
          <button className="primary-button update-primary" onClick={() => void prepare()} type="button">Check update</button>
        </div>
      ) : null}
      ...
      {phase === 'preparing' ? (
        <div className="update-progress" role="status">
          ...
          <div><strong>Preparing a fresh update plan…</strong><p>Capturing rollback state and validating the current remote configuration.</p></div>
        </div>
      ) : null}
      ...
      {snapshot && plan && strategy && (phase === 'review' || phase === 'intent-creating') ? (
        <div className="update-review">
          <div className="update-section-heading">
            <div><span className="update-step">1</span><div><strong>Server-generated plan</strong><p>Snapshot {formatTimestamp(snapshot.createdAt)}</p></div></div>
            <span className={`status-pill ${eligibility?.executable ? 'status-ok' : 'status-muted'}`}>{eligibility?.executable ? 'Executable' : 'Blocked'}</span>
          </div>
          ...
          <div className="update-warning" role="note">
            <strong>Rollback boundary</strong><p>{plan.modelVolumeBackup.warning}</p>
          ...
          <div className={`update-eligibility ${eligibility?.executable ? 'update-eligibility-ready' : 'update-eligibility-blocked'}`}>
            <strong>{eligibility?.executable ? 'Ready for revalidation' : 'Update cannot execute'}</strong><span>{eligibility?.message}</span>
          </div>
          <div className="update-action-row update-action-row-end">
            ...
            {eligibility?.executable ? (
              <button className="primary-button update-primary" disabled={phase === 'intent-creating'} onClick={() => void createIntent()} type="button">{phase === 'intent-creating' ? 'Revalidating…' : 'Prepare confirmation'}</button>
      ...
      {phase === 'confirm' && intent && plan ? (
        <div className="update-confirm">
          <div className="update-section-heading">
            <div><span className="update-step">2</span><div><strong>Confirm the revalidated transaction</strong><p>Intent created {formatTimestamp(intent.createdAt)}</p></div></div>
            <span className="status-pill status-muted">Mutation</span>
          </div>
          <p className="update-small">The server recomputed the plan and Compose strategy before issuing this intent. Execution is pinned to the candidate below.</p>
          <dl className="update-metrics">
          ...
          <div className="update-warning update-warning-strong" role="alert"><strong>Model data volumes are not backed up</strong><p>{plan.modelVolumeBackup.warning}</p></div>
          <label className="update-check">
            <input checked={rollbackAcknowledged} onChange={(event) => setRollbackAcknowledged(event.target.checked)} type="checkbox" />
          ...
          <label className="update-confirm-name">
            ...
            <input autoComplete="off" onChange={(event) => setTypedTargetName(event.target.value)} placeholder={target.displayName} spellCheck={false} value={typedTargetName} />
          </label>
          <div className="update-action-row update-action-row-end">
            <button className="secondary-button" onClick={reset} type="button">Cancel</button>
            <button className="danger-button" disabled={!confirmationReady} onClick={() => void execute()} type="button">Execute update</button>
          </div>
        </div>
      ) : null}
      ...
      {phase === 'executing' ? (
        <div className="update-progress update-progress-danger" role="status">
          <span className="update-spinner" aria-hidden="true" />
          <div><strong>Update transaction in progress…</strong><p>The server is performing an exact-digest replacement, health verification and automatic rollback if required. Do not retry this mutation automatically.</p></div>
        </div>
      ) : null}

      {phase === 'success' && result ? (
        <div className="update-success" role="status">
          <div><strong>Update completed and health-verified</strong><p>Target status has been refreshed. The new container binding is active.</p></div>
          <dl className="update-metrics compact">
      ...
      {phase === 'error' && error ? (
        <div className="update-error" role="alert">
          <strong>Update workflow stopped</strong><p>{error}</p>
