...
describe('web API client', () => {
  ...
  it('maps structured API errors without exposing an unrelated response body', async () => {
    globalThis.fetch = vi.fn(async () => jsonResponse(409, {
      error: { code: 'JOB_CONFLICT', message: 'Another mutation is active.' },
      ignored: 'REMOTE-SECRET',
    })) as typeof fetch;

    await expect(api.listTargets()).rejects.toEqual(
      expect.objectContaining<ApiError>({ status: 409, code: 'JOB_CONFLICT', message: 'Another mutation is active.' }),
    );
  });
});
